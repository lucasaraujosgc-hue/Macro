...
export default function Execution() {
  ...
  useEffect(() => {
    if (
      ...
      execution.currentStepIndex > lastStepRef.current
    ) {
      lastStepRef.current = execution.currentStepIndex;
      const iframe = document.getElementById(
        "execution-proxy-iframe",
  ...
  if (!execution) {
    return (
      <div className="flex items-center justify-center p-12 text-slate-500 text-sm backdrop-blur-md bg-white/5 border border-white/10 rounded-xl shadow-xl shadow-black/10">
        Nenhuma execução ativa no momento. Inicie uma Macro na página de
        Automações.
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-3 bg-black/20 px-3 py-1.5 rounded-full border border-white/5 flex-wrap">
          ...
          {execution.status === "running" && (
            <span className="inline-flex items-center px-2 py-0.5 rounded border border-indigo-500/30 text-[10px] font-bold uppercase tracking-wider bg-indigo-500/20 text-indigo-400">
              <span className="w-1.5 h-1.5 bg-indigo-400 rounded-full animate-pulse mr-1.5"></span>{" "}
              Em andamento
            </span>
          )}
          {execution.status === "paused" && (
            <span className="inline-flex items-center px-2 py-0.5 rounded border border-amber-500/30 text-[10px] font-bold uppercase tracking-wider bg-amber-500/20 text-amber-400">
              <span className="w-1.5 h-1.5 bg-amber-400 rounded-full mr-1.5"></span>{" "}
              Pausado (Aguardando)
            </span>
          )}
          {execution.status === "completed" && (
            <span className="inline-flex items-center px-2 py-0.5 rounded border border-green-500/30 text-[10px] font-bold uppercase tracking-wider bg-green-500/20 text-green-400">
          ...
          <div className="flex-1 space-y-1.5 overflow-y-auto">
            {execution.logs.map((l: string, i: number) => {
              ...
              if (
                ...
                l.toLowerCase().includes("paused") ||
                l.toLowerCase().includes("captcha") ||
                l.toLowerCase().includes("certificado")
              )
              ...
              else if (l.toLowerCase().includes("executing"))
            ...
            {execution.status === "paused" && execution.screenshot && (
              <div className="absolute inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-6">
                <div className="w-full max-w-sm backdrop-blur-xl bg-amber-500/10 p-6 rounded-xl border border-amber-500/20 border-t-4 border-t-amber-500 shadow-2xl flex flex-col pt-5 relative overflow-hidden">
                  ...
                  <h3 className="font-bold text-amber-400 mb-1 tracking-tight">
                    Intervenção Manual
                  </h3>
                  <p className="text-xs text-slate-400 mb-6 leading-relaxed bg-black/50 p-2 rounded border border-white/5">
                    Resolva o obstáculo para prosseguir.
                  </p>
                  ...
                  <div className="border border-white/10 rounded-lg p-2 mb-6 flex justify-center bg-black/40 shadow-inner">
                    <img
                      ...
                      alt="Captcha"
                      className="max-w-full h-auto rounded opacity-80 mix-blend-screen"
                    />
                  </div>
