...
export default function Gallery() {
  ...
  const filteredFiles = files.filter((f) => {
    ...
    const matchDate = filterDate
      ? new Date(f.createdAt).toISOString().split("T")[0] === filterDate
      : true;
    const matchSearch = filterDate ? true : true; // handled below
    const searchLow = searchQuery.toLowerCase();
    const matchText =
      f.filename.toLowerCase().includes(searchLow) ||
      getCompanyInfo(f.companyId).toLowerCase().includes(searchLow);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-400">
            Gerencie e visualize arquivos e relatórios baixados pelas
            automações.
          </p>
        </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-6 shadow-lg flex flex-wrap gap-4 items-end">
        <div className="flex-1 min-w-[200px]">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-black/20 border border-white/10 rounded-lg text-sm focus:border-indigo-500 outline-none transition-colors"
            />
          </div>
        </div>
        ...
        <div className="w-48">
          ...
          <div className="relative">
            <input
              ...
              className="w-full px-3 py-2 bg-black/20 border border-white/10 rounded-lg text-sm tracking-widest text-slate-300 focus:border-indigo-500 outline-none css-date-icon-hidden"
            />
          </div>
        </div>
        ...
        <div className="w-56">
          ...
          <select
            ...
            className="w-full px-3 py-2 bg-black/20 border border-white/10 rounded-lg text-sm text-slate-300 focus:border-indigo-500 outline-none appearance-none"
          >
        ...
        {filteredFiles.map((file) => {
          ...
          return (
            ...
            >
              ...
              <h3
                className="font-semibold text-white truncate mb-1 pr-6"
                title={file.filename}
              >
                {file.filename}
              </h3>
              ...
              <div className="mt-auto pt-4 flex items-center border-t border-white/5">
                <span className="text-xs uppercase tracking-wider font-semibold text-slate-500 truncate">
                  {getCompanyInfo(file.companyId)}
                </span>
              </div>
            </div>
          );
